...
With the Minikube Docker driver on Windows, the Minikube internal IP is not directly reachable from the host. Start a local forward in a separate terminal:
```bash
kubectl port-forward service/app-recreate-service 30040:80
```

Keep that command running, then test access via local port 30040:
```bash
...
If the port-forward prints `network namespace ... is closed` or `lost connection to pod`, this is expected: the Recreate strategy terminated the pod being forwarded. Restart the port-forward after the new pods become Ready, then run `curl http://localhost:30040` again.
